...
2. **A developer shell with `NITRO_PORT`, `NITRO_HOST` or a real `DATABASE_URL` exported, running `npm run test:http`.** Expected: the server under test still binds `127.0.0.1:<ephemeral>` and never touches that database. Test: Task 4, `harness.test.mjs` "ambient … leave the server on 127.0.0.1:<ephemeral> with PGlite".
