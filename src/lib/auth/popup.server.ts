...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  if (done) {
    ...
    const errored = Boolean(errorCode);
    const token = errored ? null : readSessionToken(request);
    const detail = [errorCode, errorDescription]
      .filter((value): value is string => Boolean(value) && value !== "1")
      .join(" ");
    const message: PopupMessage = {
      source: "grok-auth-popup",
      token,
      ...(errored ? { error: detail || errorCode || "sign_in_failed" } : {}),
    };
  ...
  try {
    ...
    if (!requestHeaders.get("x-forwarded-proto") && origin.startsWith("https://")) {
      requestHeaders.set("x-forwarded-proto", "https");
    }
    if (!requestHeaders.get("origin")) requestHeaders.set("origin", origin);
    ...
    if (!apiRes.ok && !(apiRes.status >= 300 && apiRes.status < 400)) {
    ...
    const headerLocation = apiRes.headers.get("location");
    const body = (await apiRes.json().catch(() => null)) as {
      url?: string;
    } | null;
    const location = headerLocation || body?.url;
    if (!location) {
      return completionResponse({
        source: "grok-auth-popup",
        token: null,
...
/** Public https origin for sandbox hosts even if the request URL is http. */
function publicOrigin(request: Request, url: URL): string {
  const forwardedHost = request.headers.get("x-forwarded-host");
  const host = (forwardedHost || url.host).split(",")[0]?.trim() || url.host;
  const hostName = host.split(":")[0] ?? host;
  const forwardedProto = request.headers.get("x-forwarded-proto")?.split(",")[0]?.trim();
  const sandbox =
    hostName === "grok-sandbox.com" || hostName.endsWith(".grok-sandbox.com");
  const proto = forwardedProto || (sandbox ? "https" : url.protocol.replace(":", "") || "https");
  return `${proto}://${host}`;
}
...
function readSessionToken(request: Request): string | null {
  ...
  for (const name of names) {
    ...
    const chunk = readCookie(request, `${name}.0`);
