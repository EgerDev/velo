 ...
 *   - Sandbox live preview: no `GROK_AUTH_*` injection -> no federated providers
 *     (the committed preview client was removed); derives the preview's
 *     `https://*.grok-sandbox.com` origin from the request. Sessions
 ...
 *
 * NEVER import this from client code — it pulls in `pg` + auth secrets +
...
const PREVIEW_ALLOWED_HOSTS = ["*.grok-sandbox.com"] as const;
...
// Broker federation creds: the deployer injects a per-app client. No fallback —
// without them federated sign-in is off, and production refuses to boot.
