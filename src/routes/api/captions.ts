  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        // A caption lookup fans out to up to ~14 InnerTube calls, so an unmetered
        // flood of attacker-chosen ids can get the server IP rate-banned by
        // YouTube. Clear the cheap per-IP backstop first.
